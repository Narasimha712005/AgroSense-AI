...
If the selected mode is missing credentials, emails fall back to console
logging so the application never crashes on email delivery.
"""
...
def _build_html(title: str, body: str, button_text: str, button_url: str) -> str:
    """HTML template for emails with an action button (e.g. password reset)."""
    return f"""
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;
      ...
      <a href="{button_url}"
         style="display:inline-block;margin:24px 0;background:#22c55e;color:#ffffff;
                text-decoration:none;padding:12px 32px;border-radius:10px;font-weight:bold">
        {button_text}
      </a>
      <p style="color:#64748b;font-size:12px">
        If the button doesn't work, copy this link:<br>{button_url}
      </p>
      <p style="color:#64748b;font-size:12px">
...
def _build_otp_html(username: str, otp: str) -> str:
    ...
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;
      ...
      <p style="color:#94a3b8;line-height:1.6">
        Hello {username},<br><br>Your email verification OTP is:
      </p>
...
def _send_smtp(to_email: str, subject: str, html: str) -> bool:
    """Send via SMTP (Gmail) with STARTTLS. Detailed step-by-step logging;
    ...
    try:
        ...
        with smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT, timeout=20) as server:
            ...
            logger.info("SMTP: login success. Sending email to %s ...", to_email)
        ...
        logger.error(
            "SMTP: authentication failed for %s - check the Gmail App Password. (%s)",
...
def _send(to_email: str, subject: str, html: str, url: str) -> bool:
    ...
    logger.info("Email mode selected: %s", mode)
    ...
    if mode != "console":
        logger.warning(
            "Email mode '%s' is missing credentials - using console fallback", mode
        )
...
def send_password_reset_email(to_email: str, username: str, token: str) -> bool:
    ...
    html = _build_html(
        ...
        body=(
            f"Hi {username}, we received a request to reset your AgroSense AI "
